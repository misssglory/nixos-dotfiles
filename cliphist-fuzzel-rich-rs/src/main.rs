...
fn current_buffer_prompt() -> String {
    match Command::new("wl-paste").arg("-n").output() {
        Ok(out) if out.status.success() => {
            let s = String::from_utf8_lossy(&out.stdout).to_string();
            let (l, w, c) = count_text(&s);
            format!("buf L{} W{} C{} > ", fmt_sci(l), fmt_sci(w), fmt_sci(c))
        }
        _ => "buf L0 W0 C0 > ".to_string(),
    }
}
...
fn main() -> Result<()> {
    ...
    let prompt = current_buffer_prompt();
    ...
    let item = String::from_utf8_lossy(&out.stdout).trim().to_string();
            ...
            if !item.is_empty() {
                ...
                if let Some(e) = line {
                    if is_binary_preview(&e.raw_preview) {
                        ...
                        write_clipboard(merged.as_bytes())?;
                    }
                }
            }
        }
